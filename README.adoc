...
```bash
sed -i -E "s/^(HOOKS).*/\1=(base systemd keyboard autodetect modconf sd-vconsole kms block sd-encrypt filesystems fsck)/" /etc/mkinitcpio.conf
mkinitcpio -P
```
...
```bash
useradd -m -G wheel,docker,uucp bojan
passwd bojan
