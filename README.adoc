...
```bash
sed -i -E "s/^(HOOKS).*/\1=(base systemd keyboard autodetect microcode modconf kms sd-vconsole sd-encrypt block filesystems fsck)/" /etc/mkinitcpio.conf
...
sed -i '/^#en_SE.UTF-8 UTF-8/s/^#//' /etc/locale.gen
locale-gen
{
    echo 'LANG=en_SE.UTF-8'
...
```bash
{
    ...
    echo
    echo 'account proton'
    echo 'host smtp.protonmail.ch'
    echo 'port 587'
    echo 'user no-reply@bojanstipic.com'
    echo 'password example'
    echo 'from no-reply@bojanstipic.com'
    echo
    echo 'account default : proton'
} | sudo tee /etc/msmtprc
```
...
```bash
{
    echo 'lockfile                   /var/lock/btrbk.lock'
    echo 'transaction_syslog         local0'
    echo 'stream_buffer              256m'
    echo 'stream_compress            zstd'
    echo
    echo 'snapshot_create            onchange'
    echo 'snapshot_dir               /snapshots/'
    echo
    echo 'subvolume    /'
    echo '    snapshot_name root'
    echo '    snapshot_preserve_min      1d'
    echo '    snapshot_preserve          0h 7d'
    echo
    echo 'subvolume    /home'
    echo '    snapshot_preserve_min      1d'
    echo '    snapshot_preserve          0h 7d'
    ...
    echo 'subvolume    /data'
    echo '    snapshot_preserve_min      1d'
    echo '    snapshot_preserve          0h 7d 4w 12m 5y'
} | sudo tee /etc/btrbk/btrbk.conf
...
```bash
sudo sed -i -E 's/^(DEVICESCAN)$/\1 -a -o on -S on -n standby,q -s (S/../.././02|L/../../6/03) -m inbox@bojanstipic.com/' /etc/smartd.conf
...
....
Copyright (C) 2022-2026 Bojan Stipic
